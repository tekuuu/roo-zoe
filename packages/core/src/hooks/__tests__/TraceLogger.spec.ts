...
describe("TraceLogger", () => {
	...
	it("should handle concurrent logExecution calls without losing entries", async () => {
		const filePath = path.join(workspace, "concurrent.txt")
		...
		for (let i = 0; i < 5; i++) {
			contexts.push({
				intent_id: "INT-123",
				tool_name: "write_file",
				tool_args: { path: filePath, content: `v${i}` },
			})
		}

		await Promise.all(contexts.map((ctx) => logExecution(ctx, {})))
		...
		// ensure each entry corresponds to one of the contexts
		const mutationClasses = entries.map((e: any) => e.mutation_class)
		mutationClasses.forEach((c) => expect(c).toBe("AST_REFACTOR"))
	})
})
